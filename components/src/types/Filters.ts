    ...
    constructor(params?: ISelectedFilters) {
        if (params) {
            const subject = params.Subject
                ? Map(params.Subject).map((f) => new SelectedFilter(f))
                : Map<string, SelectedFilter>();
            const study = params.Study
                ? Map(params.Study).map((f) => new SelectedFilter(f))
                : Map<string, SelectedFilter>();
            const data = Map({
                Assay:
                    params.Data && params.Data.Assay
                        ? Map(params.Data.Assay).map((f) => new SelectedFilter(f))
                        : Map<string, SelectedFilter>(),
                Timepoint:
                    params.Data && params.Data.Timepoint
                        ? Map(params.Data.Timepoint).map((f) => new SelectedFilter(f))
                        : Map<string, SelectedFilter>(),
                SampleType:
                    params.Data && params.Data.SampleType
                        ? Map(params.Data.SampleType).map((f) => new SelectedFilter(f))
                        : Map<string, SelectedFilter>(),
            });
